...
Triton Runner supports multi-level debugging across Python (Triton), TTIR, and TTGIR on Triton **v3.4.0**.
...
You can run your Triton code using `@triton_runner.jit` instead of `@triton.jit`. See an example in [examples/python_runner/matmul.py](./examples/python_runner/matmul.py#L12)
...
Similar to the TTIR runner, you need to provide a `.ttgir` file and specify its location in the program. See an example in [examples/ttgir_runner/sm90/matmul-with-tma-v4.py](./examples/ttgir_runner/sm90/matmul-with-tma-v4.py#L76).
