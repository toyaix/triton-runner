...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    BLOCK_SIZE = 1024
    debug_tensor = torch.empty_like(output)
    ...
    debug_off = BLOCK_SIZE
    triton_runner.color_print.blue_print(f"debug {debug_tensor[debug_off: debug_off + BLOCK_SIZE]}")
    max_diff = torch.max(torch.abs(debug_torch[block_start: block_start + BLOCK_SIZE] - debug_tensor[debug_off: debug_off + BLOCK_SIZE]))
