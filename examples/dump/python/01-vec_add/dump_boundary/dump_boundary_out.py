...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    BLOCK_SIZE = 1024
    dump_tensor = torch.empty(BLOCK_SIZE, dtype=torch.float32, device=x.device)
    ...
    dump_torch = x + y
    boundary_start = n_elements & (~(BLOCK_SIZE-1))
    max_diff = torch.max(torch.abs(dump_torch[boundary_start:] - dump_tensor[:n_elements-boundary_start]))
