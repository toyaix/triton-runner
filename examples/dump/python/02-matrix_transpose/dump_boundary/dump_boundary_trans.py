...
def solve(input: torch.Tensor, output: torch.Tensor, rows: int, cols: int):
    ...
    dump_torch = output
    boundary_start_rows = rows & (~(BLOCK_SIZE-1))
    boundary_start_cols = rows & (~(BLOCK_SIZE-1))
    max_diff = torch.max(torch.abs(dump_torch[boundary_start_cols:, boundary_start_rows:] - dump_tensor[:cols-boundary_start_cols, :rows-boundary_start_rows]))
