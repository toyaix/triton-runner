...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, sinks, sm_scale, bandwidth, start_q):
        ...
        n_elements = triton_runner.torch_utils.get_n_elements_with_grid(block_shape, grid)
        dump_tensor = torch.empty(n_elements, dtype=torch.float32, device=q.device)
        ...
        dump_tensor = dump_tensor[:BLOCK_M*HEAD_DIM_K].view(BLOCK_M, HEAD_DIM_K)
