...
def matmul(a, b):
    ...
    BLOCK_SIZE_N, BLOCK_SIZE_K = 128, 256
    dump_tensor = torch.empty((BLOCK_SIZE_N, BLOCK_SIZE_K//2), dtype=torch.float32, device=a.device)
    ...
    dump_torch = b
    max_diff = torch.max(torch.abs(dump_torch[K//2 - BLOCK_SIZE_K//2:K//2, :BLOCK_SIZE_N] - dump_tensor))
...
# copy from https://github.com/meta-pytorch/tritonbench/blob/main/tritonbench/operators/int4_gemm/int4_gemm.py
def triton_int4_gemm(x, w):
    x_2d = x.reshape(-1, x.size(-1))
    w_int4_packed = pack_2xint4(w).T.contiguous().T

    return matmul(x_2d, w_int4_packed)

def _matmul_from_packed(x_2d, w_int4_packed, K, N):
...
def eager_int4_gemm(x, w):
    x_2d = x.reshape(-1, x.size(-1))
    K, N = w.shape
    w_int4_packed = pack_2xint4(w).T.contiguous().T
    return  _matmul_from_packed(x_2d, w_int4_packed, K, N)

B, L, Dout, Din = 16, 4096, 7168, 8192

x = torch.randn(B, L, Din, device="cuda", dtype=torch.bfloat16)
w = torch.randint(-8, 7, (Din, Dout), device="cuda", dtype=torch.int32)
torch_output = eager_int4_gemm(x, w)
triton_output = triton_int4_gemm(x, w)
