...
def add_kernel(x_ptr,  # *Pointer* to first input vector.
    ...
    if (pid_x == 0 and pid_y == 0) and pid_z == 0:
        ...
        # only store in tid.x=0
        tl.store(dump_tensor + blockdim + tid, blockdim)
...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    BLOCK_SIZE = 2
    dump_tensor = torch.empty((2048), dtype=x.dtype, device=x.device)
    ...
    triton_runner.color_print.blue_print(f"debug {dump_tensor[0:128]}")
    triton_runner.color_print.blue_print(f"debug {dump_tensor[128:]}")
    print(dump_tensor[256])
    # dump_torch = x + y
