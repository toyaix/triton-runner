...
def is_sm100():
    # The scatter4 TMA instruction is only available on sm100. sm120 (consumer
    # Blackwell) supports gather4 but not scatter4, so async_scatter and the
    # fused gather+scatter matmul below only run on sm100.
    target = triton.runtime.driver.active.get_current_target()
    return target.backend == "cuda" and torch.cuda.get_device_capability()[0] == 10
...
@pytest.mark.parametrize("X_MAX, Y_MAX", [(1024, 1024)])
@pytest.mark.skipif(not is_sm100(), reason="Requires sm100 (scatter4 is not available on sm120)")
...
def matmul_fused_gather_scatter_kernel(X_desc, W_desc, out_desc, X_gather_indx_ptr, out_scatter_indx_ptr,
                                       BLOCK_M: gl.constexpr, SchedulerImpl: gl.constexpr, MMAImpl: gl.constexpr,
    ...
    mma = MMAImpl.initialize(dtype, BLOCK_M, BLOCK_N, gl.num_warps())
...
def matmul_fused_gather_scatter(X, X_gather_indx, W, out_scatter_indx, BLOCK_M=128, BLOCK_N=128, BLOCK_K=64,
    ...
    matmul_fused_gather_scatter_kernel[grid](X_desc, W_desc, out_desc, X_gather_indx, out_scatter_indx, BLOCK_M,
                                             SchedulerImpl, MMAImpl, num_buffers)
    return out
