...
def get_1d_injected_ir(ssa_value, original_line, indent, size, encoding, loc, python_dump, offset_val, replace_id):
...
def get_2d_injected_ir_without_encoding(ssa_value, original_line, indent, size, loc, python_dump, offset_val, replace_id):
    ir_indent = indent if python_dump else f"{indent}  "
    off_ir = f"{offset_val}" if python_dump else f"arith.constant 0 : i32 {loc}"
    id_str = f"_{replace_id}" if python_dump else f""
...
def get_injected_ir(ssa_value, op, original_line, indent, size, encoding, loc, python_dump=False, offset_val="", replace_id=0):
    loc = f"loc({loc})"
    encoding = f", {encoding}" if encoding else ""
    warning_dump_mode_ssa_and_op(ssa_value, op, loc, size, encoding)
    if size.count("x") == 0:
        return get_1d_injected_ir(ssa_value, original_line, indent, size, encoding, loc, python_dump, offset_val, replace_id)
    elif size.count("x") == 1:
        if encoding:
            return get_2d_injected_ir_with_encoding(ssa_value, original_line, indent, size, encoding, loc)
        else:
            return get_2d_injected_ir_without_encoding(ssa_value, original_line, indent, size, loc, python_dump, offset_val, replace_id)
    else:
