# copy from https://github.com/triton-lang/triton/blob/main/python/triton/testing.py
...
def do_bench(fn, warmup=25, rep=100, grad_to_none=None, quantiles=None, return_mode="mean"):
    ...
    di = triton_driver.active.get_device_interface()

    fn()
    di.synchronize()

    cache = triton_driver.active.get_empty_cache_for_benchmark()
    ...
    start_event.record()
    for _ in range(5):
        triton_driver.active.clear_cache(cache)
        fn()
    end_event.record()
    di.synchronize()
    estimate_ms = start_event.elapsed_time(end_event) / 5
...
def get_dram_gbps(device=None):
    ...
    if device is None:
        device = triton_driver.active.get_device_interface().current_device()
    mem_clock_khz = triton_driver.active.utils.get_device_properties(device)["mem_clock_rate"]  # in kHz
    bus_width = triton_driver.active.utils.get_device_properties(device)["mem_bus_width"]
...
def get_max_tensorcore_tflops(dtype, clock_rate, device=None):
    ...
    num_subcores = triton_driver.active.utils.get_device_properties(device)["multiprocessor_count"] * 4
